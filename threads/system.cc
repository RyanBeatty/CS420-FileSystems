...
Timer *timeSlicer;
SpaceId processId = 0;
ProcessList *processList; //to keep track of current processes
...
//----------------------------------------------------------------------
//  TimeSlicerInterruptHandler()
//   Interrupt handler for the timer that will provide time slicing for user
//  processes.
//----------------------------------------------------------------------
void
TimeSlicerInterruptHandler(int) {
    if (interrupt->getStatus() != IdleMode)
        interrupt->YieldOnReturn();
}
...
#ifdef USER_PROGRAM
    processList = new(std::nothrow) ProcessList();      //the data structure used to track process initialization and time-slicing
    machine = new(std::nothrow) Machine(debugUserProg); // this must come first
    synchConsole = new(std::nothrow) SynchConsole(NULL,NULL);
    bitMap = new(std::nothrow) BitMap(NumPhysPages);              //bitmap keeps track of 32 page frames in physical memory
    ioLock = new(std::nothrow) Lock("io Lock");
