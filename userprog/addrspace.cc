...
// how big is address space?
    ...
    bzero(pageMap,numPages);

    for(unsigned int i = 0; i < numPages; i++){         // iterate over the number of virtual pages and map them to page frames
        int nextFreeFrame = bitMap->Find();            // find the next free page frame in main memory
        if(nextFreeFrame == -1){
            //No free space, temporary solution
            ASSERT(false);
        }

        pageMap[i] = nextFreeFrame;                                         //  map ith virtual page to a physical page frame                                 
        bzero(machine->mainMemory+(nextFreeFrame * PageSize), PageSize);    //Zeroing out the desired frame in main memory
    }
